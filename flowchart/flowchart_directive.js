...
//
.directive('chartJsonEdit', function ($rootScope) {
	return {
		restrict: 'AC',
		scope: {
			viewModel: "="
		...
		link: function ($scope, elem, attr) {
			...
			var updateJson = function () {
				if ($scope.viewModel) {
					...
					$rootScope.$broadcast("orgChart", JSON.stringify($scope.viewModel.data))
					$(elem).val(json);
				}
			};
			...
			$(elem).bind("input propertychange", function () {
				...
				$scope.dataModel = JSON.parse(json);
...
.controller('FlowChartController', ['$scope', 'dragging', '$element', function FlowChartController ($scope, dragging, $element) {
	...
	$scope.mouseDown = function (evt) {
		...
		//Just a click anywhere but a node
	...
	$scope.mouseMove = function (evt) {
		...
		if ($scope.mouseOverNode == null) {
			var circles = document.getElementsByClassName('connector-circle')
        ...
        for (var i=0; i < amount; i++){
			...
			mouseOverElement.parentElement.children[2].children[1].style.display = ""
			mouseOverElement.parentElement.children[3].children[1].style.display = ""
		}
	};
	...
	var nextnodeID = 2;
	$scope.addNewNode = function(nodeParent, connector){
        ...
        var newNodeDataModel = {
          ...
          nextnodeID++
          console.log("adding onde")
          console.log(connector)
          $scope.chart.addNode(connector, newNodeDataModel);
	}
